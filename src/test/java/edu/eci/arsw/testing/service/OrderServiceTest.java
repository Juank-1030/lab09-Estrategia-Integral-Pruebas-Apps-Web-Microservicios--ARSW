...
class OrderServiceTest {
    ...
    void shouldReturnOrderWhenFindByIdExists() {
        ...
        Order existingOrder = new Order("ORD-123", "CUS-05", 50000, "CREATED", Instant.now());
        when(repository.findById("ORD-123")).thenReturn(Optional.of(existingOrder));

        OrderResponse response = service.findById("ORD-123");

        assertNotNull(response);
        assertEquals("ORD-123", response.id());
        assertEquals("CUS-05", response.customerId());
        assertEquals(50000, response.total());
        assertEquals("CREATED", response.status());
        verify(repository, times(1)).findById("ORD-123");
    }
